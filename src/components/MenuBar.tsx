...
const MenuBar = ({ TelaAtiva }:defaultProps) => {
    ...
    return (
        <View style={styles.menu}>
            <TouchableOpacity onPress={() => navigation.navigate('Home' as never)} style={{ alignItems: 'center' }}>
                <FontAwesome name="home" size={23} color={TelaAtiva == 'home' ? '#E96181' : '#C8C8C8'} />
                <Text style={{ fontSize: 12 }}>Home</Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={()=> navigation.navigate('Activitys' as never)} style={{ alignItems: 'center' }}>
                <FontAwesome name="list-ul" size={23} color={TelaAtiva == 'atividade' ? '#E96181' : '#C8C8C8'} />
                <Text style={{ fontSize: 12 }}>Atividade</Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={()=> navigation.navigate('PreChat' as never)} style={{ alignItems: 'center' }}>
                <FontAwesome name="wechat" size={23} color={TelaAtiva == 'chat' ? '#E96181' : '#C8C8C8'} />
                <Text style={{ fontSize: 12 }}>Chat</Text>
            </TouchableOpacity>

            <TouchableOpacity onPress={()=> navigation.navigate('Settings' as never)} style={{ alignItems: 'center' }}>
